...
from src.utils.chat_logger import (
    log_workout_message, log_diet_message, log_reminder_message, 
    log_progress_message, log_completion_message, log_general_message
)
...
class BotHandlers:
    ...
    async def handle_schedule_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if reminder_count > 0:
                # Log reminder setup message
                reminder_setup_message = f"⏰ **Reminders Set!**\n\nI've created {reminder_count} reminders for today's schedule:\n• 💪 Workout at {user.workout_time}\n• 🍳 Breakfast at {user.breakfast_time}\n• 🍽️ Lunch at {user.lunch_time}\n• 🍴 Dinner at {user.dinner_time}\n• 🍎 Snack at {user.snack_time}\n\nYou'll receive notifications 5 minutes before each activity! 🎯"
                log_reminder_message(
                    user_id=user_id,
                    message_text=reminder_setup_message,
                    chat_id=update.effective_chat.id,
                    reply_to_message_id=update.message.message_id
                )
                await update.message.reply_text(reminder_setup_message, parse_mode='Markdown')
            else:
                await update.message.reply_text(
